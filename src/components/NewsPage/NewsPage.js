...
const newspage = props => { // component rendering the news list
  let NewsList;
  if (props.data) {
       ...
       // console.log("=== i", i)
        ...
        return (
          <div className={styles.mainStyle} key={data.objectID}>
            ...
            <div className={styles.titleFont} key={data.url}>
              {data.title} <span className={styles.titleURL}>({domain}) </span>
            </div>
            <br />
            <div className={styles.hideAlign}>
              <div className={styles.commentFont}>
                {data.points} points By {data.author} {" | "} {data.num_comments}
               {" "} comments {"| "} {diffDays} hours ago
              </div>
              <div
